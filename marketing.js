...
// ════════════════════════════════════════════════════════════
// VIEW 5: 战略矩阵 (Strategic Matrices)
// ════════════════════════════════════════════════════════════

// BCG quadrant scatter plot builder
function bcgPlot(items,title,subtitle){
    // items: [{name, share, growth, size, color}]
    // share: 0-100 (x), growth: -100 to 200 (y), size: bubble size
    const maxSize=Math.max(...items.map(i=>i.size),1);
    const qColors={star:'rgba(16,185,129,.12)',cow:'rgba(59,130,246,.12)',question:'rgba(245,158,11,.12)',dog:'rgba(239,68,68,.08)'};
    const qLabels={star:'Stars ★',cow:'Cash Cows 🐄',question:'Question Marks ❓',dog:'Dogs 🐕'};
    // Map to pixel positions within 100% container
    // X: share 0-100 maps to 0-100%
    // Y: growth maps inversely (high growth = top)
    const growths=items.map(i=>i.growth);
    const maxG=Math.max(...growths,10);
    const minG=Math.min(...growths,-10);
    const gRange=maxG-minG||1;
    const midG=(maxG+minG)/2;
    const midX=50;

    return`<div class="panel panel-gold"><div class="panel-h"><span class="panel-t">${title}</span><span class="panel-badge">${subtitle}</span></div><div class="panel-b">
        <div style="position:relative;width:100%;height:340px;border:1px solid var(--border);border-radius:8px;overflow:hidden">
            <div style="position:absolute;left:0;top:0;width:50%;height:50%;background:${qColors.question}"></div>
            <div style="position:absolute;right:0;top:0;width:50%;height:50%;background:${qColors.star}"></div>
            <div style="position:absolute;left:0;bottom:0;width:50%;height:50%;background:${qColors.dog}"></div>
            <div style="position:absolute;right:0;bottom:0;width:50%;height:50%;background:${qColors.cow}"></div>
            <div style="position:absolute;left:50%;top:0;bottom:0;width:1px;background:var(--border2)"></div>
            <div style="position:absolute;top:50%;left:0;right:0;height:1px;background:var(--border2)"></div>
            <div style="position:absolute;left:4px;top:4px;font-size:9px;color:var(--orange);font-weight:600;opacity:.7">${qLabels.question}</div>
            <div style="position:absolute;right:4px;top:4px;font-size:9px;color:var(--green);font-weight:600;opacity:.7">${qLabels.star}</div>
            <div style="position:absolute;left:4px;bottom:4px;font-size:9px;color:var(--red);font-weight:600;opacity:.7">${qLabels.dog}</div>
            <div style="position:absolute;right:4px;bottom:4px;font-size:9px;color:var(--blue);font-weight:600;opacity:.7">${qLabels.cow}</div>
            <div style="position:absolute;bottom:-2px;left:50%;transform:translateX(-50%);font-size:8px;color:var(--text3)">← 低份额 | 高份额 →</div>
            <div style="position:absolute;left:2px;top:50%;transform:translateY(-50%) rotate(-90deg);font-size:8px;color:var(--text3);white-space:nowrap">← 低增长 | 高增长 →</div>
            ${items.map(item=>{
                const x=Math.max(5,Math.min(95,item.share));
                const y=Math.max(5,Math.min(95,100-((item.growth-minG)/gRange*100)));
                const r=Math.max(16,Math.min(50,item.size/maxSize*50));
                const q=item.share>=midX?(item.growth>=midG?'star':'cow'):(item.growth>=midG?'question':'dog');
                const qc={star:'var(--green)',cow:'var(--blue)',question:'var(--orange)',dog:'var(--red)'}[q];
                return`<div style="position:absolute;left:${x}%;top:${y}%;transform:translate(-50%,-50%);width:${r}px;height:${r}px;border-radius:50%;background:${qc};opacity:.8;border:2px solid ${qc};display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:2" title="${item.name}: 份额${item.share.toFixed(0)}%, 增长${item.growth.toFixed(0)}%, ${item.size}件"><span style="font-size:${r>30?9:7}px;font-weight:700;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:${r-4}px">${item.name.slice(0,4)}</span></div>`}).join('')}
        </div>
        <div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px">${items.map(item=>{
            const q=item.share>=midX?(item.growth>=midG?'star':'cow'):(item.growth>=midG?'question':'dog');
            const qc={star:'var(--green)',cow:'var(--blue)',question:'var(--orange)',dog:'var(--red)'}[q];
            const ql={star:'★星',cow:'🐄现金牛',question:'❓问号',dog:'🐕瘦狗'}[q];
            return`<span style="font-size:10px;padding:2px 8px;border-radius:4px;background:${qc}12;color:${qc};border:1px solid ${qc}30">${item.name} <b>${ql}</b></span>`}).join('')}
        </div>
    </div></div>`;
}

// GE McKinsey 3x3 grid builder
function gePlot(items,title){
    // items: [{name, attract (1-9), strength (1-9), size}]
    // 3x3: high/mid/low attract × strong/mid/weak strength
    const cells=[
        {r:0,c:0,label:'选择性投资',bg:'rgba(245,158,11,.1)',action:'评估后投'},
        {r:0,c:1,label:'重点投资',bg:'rgba(16,185,129,.12)',action:'加大投入'},
        {r:0,c:2,label:'优先投资',bg:'rgba(16,185,129,.2)',action:'全力投入'},
        {r:1,c:0,label:'审慎管理',bg:'rgba(239,68,68,.06)',action:'维持观望'},
        {r:1,c:1,label:'选择性投资',bg:'rgba(245,158,11,.1)',action:'精选投入'},
        {r:1,c:2,label:'重点投资',bg:'rgba(16,185,129,.12)',action:'加大投入'},
        {r:2,c:0,label:'收割/退出',bg:'rgba(239,68,68,.12)',action:'减少投入'},
        {r:2,c:1,label:'审慎管理',bg:'rgba(239,68,68,.06)',action:'维持'},
        {r:2,c:2,label:'选择性投资',bg:'rgba(245,158,11,.1)',action:'评估后投'}
    ];
    // Map items to cells: attract high(7-9)=row0, mid(4-6)=row1, low(1-3)=row2
    // strength weak(1-3)=col0, mid(4-6)=col1, strong(7-9)=col2
    const maxSize=Math.max(...items.map(i=>i.size),1);
    const placed=items.map(i=>{
        const r=i.attract>=7?0:i.attract>=4?1:2;
        const c=i.strength>=7?2:i.strength>=4?1:0;
        const cell=cells.find(cl=>cl.r===r&&cl.c===c);
        return{...i,row:r,col:c,action:cell?cell.action:'?'};
    });

    return`<div class="panel"><div class="panel-h"><span class="panel-t">${title}</span><span class="panel-badge">GE-McKinsey 9格</span></div><div class="panel-b">
        <div style="display:grid;grid-template-columns:40px 1fr 1fr 1fr;grid-template-rows:auto repeat(3,90px) auto;gap:1px">
            <div></div><div style="text-align:center;font-size:9px;color:var(--text3);padding:2px">弱竞争力</div><div style="text-align:center;font-size:9px;color:var(--text3);padding:2px">中竞争力</div><div style="text-align:center;font-size:9px;color:var(--text3);padding:2px">强竞争力</div>
            ${[0,1,2].map(r=>{
                const rowLabel=['高吸引','中吸引','低吸引'][r];
                return`<div style="writing-mode:vertical-rl;text-orientation:mixed;font-size:9px;color:var(--text3);display:flex;align-items:center;justify-content:center">${rowLabel}</div>${[0,1,2].map(c=>{
                    const cell=cells.find(cl=>cl.r===r&&cl.c===c);
                    const cellItems=placed.filter(p=>p.row===r&&p.col===c);
                    return`<div style="background:${cell.bg};border-radius:6px;padding:4px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;min-height:80px"><div style="font-size:8px;color:var(--text3);font-weight:600">${cell.label}</div>${cellItems.map(ci=>`<div style="background:var(--surface);border:1px solid var(--border);border-radius:4px;padding:2px 6px;font-size:10px;font-weight:600;white-space:nowrap">${ci.name} <span style="color:var(--text3)">${ci.size}</span></div>`).join('')}</div>`}).join('')}`}).join('')}
        </div>
        <div style="margin-top:10px"><table class="tbl" style="font-size:11px"><thead><tr><th>项目</th><th class="n">吸引力</th><th class="n">竞争力</th><th class="n">规模</th><th>决策</th></tr></thead>
        <tbody>${placed.map(p=>`<tr><td style="font-weight:600">${p.name}</td><td class="n">${p.attract.toFixed(1)}</td><td class="n">${p.strength.toFixed(1)}</td><td class="n">${p.size}</td><td><span class="badge ${p.action.includes('全力')||p.action.includes('加大')?'b-green':p.action.includes('减少')||p.action.includes('退出')?'b-red':'b-orange'}">${p.action}</span></td></tr>`).join('')}</tbody></table></div>
    </div></div>`;
}
...
function vMatrix(){
    ...
    // ═══ BCG: Scenarios ═══
    const sList=[...new Set(DATA.map(d=>d.scenario).filter(Boolean))].sort();
    const scBCG=sList.map(s=>{
        const total=cnt(DATA,d=>d.scenario===s);
        const share=T?total/T*100:0;
        ...
        return{name:s,share,growth,size:total,color:SC_C[s]||'var(--text3)'};
    });

    // ═══ BCG: Channels ═══
    const chGrp=grp(DATA,'channel');
    const topCh=Object.entries(chGrp).sort((a,b)=>b[1]-a[1]).slice(0,10);
    const chBCG=topCh.map(([ch,total])=>{
        const share=T?total/T*100:0;
        const prev=cnt(DATA,d=>d.channel===ch&&d.year===prevY);
        const curr=cnt(DATA,d=>d.channel===ch&&d.year===currY);
        const growth=prev?(curr-prev)/prev*100:(curr>0?100:0);
        return{name:ch,share,growth,size:total};
    });

    // ═══ BCG: Regions ═══
    const regGrp=grp(DATA,'region');
    const topReg=Object.entries(regGrp).sort((a,b)=>b[1]-a[1]).slice(0,10);
    const regBCG=topReg.map(([r,total])=>{
        const share=T?total/T*100:0;
        const prev=cnt(DATA,d=>d.region===r&&d.year===prevY);
        const curr=cnt(DATA,d=>d.region===r&&d.year===currY);
        const growth=prev?(curr-prev)/prev*100:(curr>0?100:0);
        return{name:r,share,growth,size:total};
    });

    // ═══ GE McKinsey: Scenarios ═══
    const scMaxN=Math.max(...sList.map(s=>cnt(DATA,d=>d.scenario===s)),1);
    const scGE=sList.map(s=>{
        const items=DATA.filter(d=>d.scenario===s);
        const n=items.length;
        const countries=[...new Set(items.map(d=>(d.country||'').trim()).filter(Boolean))].length;
        const prev=items.filter(d=>d.year===prevY).length;
        const curr=items.filter(d=>d.year===currY).length;
        const growthRate=prev?(curr-prev)/prev:0;
        const attract=Math.min(9,Math.max(1, (n/scMaxN*3) + (growthRate>0?growthRate*3:growthRate*1.5) + (countries/5*3)));
        const wonN=items.filter(isWon).length;
        const wr=n?wonN/n:0;
        const channels=[...new Set(items.map(d=>d.channel).filter(Boolean))].length;
        const cuRate=n?items.filter(d=>d.customization==='〇').length/n:0;
        const strength=Math.min(9,Math.max(1, wr*4 + (n/scMaxN*3) + (channels/8*2)));
        return{name:s,attract,strength,size:n};
    });

    // ═══ Ansoff Matrix ═══
    // Existing market = country that appeared in first year; new market = appeared later
    // Existing product = scenario+hardware combo in first year; new = appeared later
    const firstYear=years[0];
    const existingCountries=new Set(DATA.filter(d=>d.year===firstYear).map(d=>(d.country||'').trim()).filter(Boolean));
    const existingScenarios=new Set(DATA.filter(d=>d.year===firstYear).map(d=>d.scenario).filter(Boolean));
    const existingHW=new Set(DATA.filter(d=>d.year===firstYear).map(d=>d.hardware1).filter(Boolean));

    let ansoff={penetration:0,marketDev:0,productDev:0,diversification:0};
    let ansoffItems={penetration:[],marketDev:[],productDev:[],diversification:[]};
    DATA.filter(d=>d.year>firstYear).forEach(d=>{
        const existMkt=existingCountries.has((d.country||'').trim());
        const existProd=existingScenarios.has(d.scenario)||existingHW.has(d.hardware1);
        if(existMkt&&existProd){ansoff.penetration++;ansoffItems.penetration.push(d)}
        else if(!existMkt&&existProd){ansoff.marketDev++;ansoffItems.marketDev.push(d)}
        else if(existMkt&&!existProd){ansoff.productDev++;ansoffItems.productDev.push(d)}
        else{ansoff.diversification++;ansoffItems.diversification.push(d)}
    });
    const ansoffTotal=ansoff.penetration+ansoff.marketDev+ansoff.productDev+ansoff.diversification;

    // New countries & scenarios introduced after first year
    const newCountries=[...new Set(DATA.filter(d=>d.year>firstYear).map(d=>(d.country||'').trim()).filter(c=>c&&!existingCountries.has(c)))];
    const newScenarios=[...new Set(DATA.filter(d=>d.year>firstYear).map(d=>d.scenario).filter(s=>s&&!existingScenarios.has(s)))];

    return`
    <div class="kpi-row c4">
        <div class="kpi kpi-gold"><div class="kpi-top"><span class="kpi-label">分析基准</span></div><div class="kpi-val gold-num">${T}</div><div class="kpi-sub">${years.join(' → ')} · ${sList.length}场景 · ${Object.keys(chGrp).length}渠道</div></div>
        <div class="kpi"><div class="kpi-top"><span class="kpi-label">增长基准期</span></div><div class="kpi-val" style="color:var(--blue)">${prevY}→${currY}</div><div class="kpi-sub">${cnt(DATA,d=>d.year===prevY)} → ${cnt(DATA,d=>d.year===currY)}件</div></div>
        <div class="kpi"><div class="kpi-top"><span class="kpi-label">新市场开拓</span></div><div class="kpi-val" style="color:var(--purple)">${newCountries.length}</div><div class="kpi-sub">个新国家 (${firstYear}后)</div></div>
        <div class="kpi"><div class="kpi-top"><span class="kpi-label">Ansoff主战略</span></div><div class="kpi-val" style="color:var(--green);font-size:16px">${ansoff.penetration>=ansoff.marketDev&&ansoff.penetration>=ansoff.productDev?'市场渗透':'市场开发'}</div><div class="kpi-sub">${ansoffTotal?pct(Math.max(ansoff.penetration,ansoff.marketDev,ansoff.productDev),ansoffTotal):0}%项目</div></div>
    </div>

    <div class="grid g2">
        ${bcgPlot(scBCG,'BCG矩阵: 场景','X=市场份额 Y=增长率 大小=项目数')}
        ${bcgPlot(chBCG,'BCG矩阵: 渠道 Top10','X=市场份额 Y=增长率 大小=项目数')}
    </div>

    <div class="grid g2" style="margin-top:12px">
        ${bcgPlot(regBCG,'BCG矩阵: 区域 Top10','X=市场份额 Y=增长率 大小=项目数')}
        ${gePlot(scGE,'GE-McKinsey: 场景投资优先级')}
    </div>

    <div class="panel" style="margin-top:12px"><div class="panel-h"><span class="panel-t">Ansoff增长战略矩阵</span><span class="panel-badge">${firstYear}为基准年 · ${ansoffTotal}个后续项目</span></div><div class="panel-b">
        <div style="display:grid;grid-template-columns:80px 1fr 1fr;grid-template-rows:auto 1fr 1fr;gap:2px">
            <div></div><div style="text-align:center;font-size:10px;font-weight:700;padding:6px;color:var(--text2)">既有产品/场景</div><div style="text-align:center;font-size:10px;font-weight:700;padding:6px;color:var(--text2)">新产品/场景</div>
            <div style="writing-mode:vertical-rl;text-orientation:mixed;font-size:10px;font-weight:700;color:var(--text2);display:flex;align-items:center;justify-content:center">既有市场</div>
            <div style="background:var(--green-bg);border-radius:8px;padding:14px;text-align:center"><div style="font-size:11px;font-weight:700;color:var(--green)">市场渗透</div><div style="font-size:28px;font-weight:800;color:var(--green);margin:4px 0">${ansoff.penetration}</div><div style="font-size:10px;color:var(--text3)">${ansoffTotal?pct(ansoff.penetration,ansoffTotal):0}% · 低风险</div><div style="font-size:9px;color:var(--text3);margin-top:4px">在已有市场深耕已有产品</div></div>
            <div style="background:var(--orange-bg);border-radius:8px;padding:14px;text-align:center"><div style="font-size:11px;font-weight:700;color:var(--orange)">产品开发</div><div style="font-size:28px;font-weight:800;color:var(--orange);margin:4px 0">${ansoff.productDev}</div><div style="font-size:10px;color:var(--text3)">${ansoffTotal?pct(ansoff.productDev,ansoffTotal):0}% · 中风险</div><div style="font-size:9px;color:var(--text3);margin-top:4px">新场景/硬件: ${newScenarios.join(', ')||'无'}</div></div>
            <div style="writing-mode:vertical-rl;text-orientation:mixed;font-size:10px;font-weight:700;color:var(--text2);display:flex;align-items:center;justify-content:center">新市场</div>
            <div style="background:var(--blue-bg);border-radius:8px;padding:14px;text-align:center"><div style="font-size:11px;font-weight:700;color:var(--blue)">市场开发</div><div style="font-size:28px;font-weight:800;color:var(--blue);margin:4px 0">${ansoff.marketDev}</div><div style="font-size:10px;color:var(--text3)">${ansoffTotal?pct(ansoff.marketDev,ansoffTotal):0}% · 中风险</div><div style="font-size:9px;color:var(--text3);margin-top:4px">新国家: ${newCountries.join(', ')||'无'}</div></div>
            <div style="background:var(--red-bg);border-radius:8px;padding:14px;text-align:center"><div style="font-size:11px;font-weight:700;color:var(--red)">多元化</div><div style="font-size:28px;font-weight:800;color:var(--red);margin:4px 0">${ansoff.diversification}</div><div style="font-size:10px;color:var(--text3)">${ansoffTotal?pct(ansoff.diversification,ansoffTotal):0}% · 高风险</div><div style="font-size:9px;color:var(--text3);margin-top:4px">新市场+新产品同时</div></div>
        </div>
    </div></div>`;
}
