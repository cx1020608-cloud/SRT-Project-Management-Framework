...
function mkGauge(val,label,color,sz){
    ...
    return`<div class="gauge-w"><svg width="${sz}" height="${sz}" viewBox="0 0 ${sz} ${sz}"><circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="rgba(255,255,255,.06)" stroke-width="${sw}"/><circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="${autoC}" stroke-width="${sw}" stroke-dasharray="${fill} ${gap}" transform="rotate(-90 ${cx} ${cy})" class="gauge-fill" style="filter:drop-shadow(0 1px 3px ${autoC}44)"/><text x="${cx}" y="${cy-1}" text-anchor="middle" dominant-baseline="central" font-size="${sz*.22}" font-weight="800" fill="var(--text)">${val}%</text></svg><div style="font-size:${Math.max(sz*.1,9)}px;color:var(--text3);text-align:center;margin-top:2px">${label}</div></div>`;
}
